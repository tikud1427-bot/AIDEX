 ...
 */
async function _resolveMemoryContext(userId, currentMessage, externalMemory) {
...
async function handleAquaRequest({
  ...
  try {
    ...
    if (safeMode === MODES.WEB_SEARCH) {
      ...
      const result  = await ai.generateSearch(messages, { userId });
    ...
    switch (intent) {
      ...
      case INTENTS.GENERATE_PROJECT: {
        ...
        try {
          await svc.createProject(userId, safeInput, pid);
        } catch { /* may already exist */ }

        const result = await svc.generateProject(userId, pid, safeInput);
        ...
        return {
          ...
          message:    `✅ Project generated with ${(result.fileData || result.files || []).length} file(s)${skippedMsg}. Open preview to see it.`,
      ...
      case INTENTS.EXPLAIN: {
        ...
        const reply = await ai.generateAI(messages, { temperature: 0.7 });
        _extractMemoryAsync(userId, safeInput);
