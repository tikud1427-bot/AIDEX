...
// ── Google sign-in ────────────────────────────────────────────────────────────
// Android cannot share Chrome's HttpOnly cookie jar with its WebView. Native
// sign-in therefore runs the Google leg entirely in Chrome, then returns a
// short-lived one-time code to the Android App Link. The WebView redeems that
// code itself, which is the request that receives the authenticated cookie.
//
// Web sign-in is unchanged: without native=1 this remains the ordinary
// passport/session flow.
app.get("/auth/google", authLimiter, (req, res, next) => {
  ...
  // The nonce is generated by the Android app and bound to this browser session.
  // Reject malformed/missing values rather than creating an unbound native flow.
  if (!/^[A-Za-z0-9_-]{20,64}$/.test(nonce)) {
    return res.status(400).send("Invalid native authentication request.");
  }
...
app.get(
  ...
  (req, res, next) => {
    const isNative = req.session?.nativeReturn === true;
    const failureRedirect = isNative
      ? `/auth/native/return?error=google_auth_failed&nonce=${encodeURIComponent(req.session.nativeNonce || "")}`
      : "/login";
  ...
  async (req, res, next) => {
    try {
      ...
      if (isNative) {
        ...
        if (!nonce) return res.redirect("/login");

        // The code is the only credential that crosses the browser → Android
        // boundary. Store only its SHA-256 hash in MongoDB.
        ...
        // These values are consumed by the native return hop, not by the WebView
        // session. Remove them before the browser follows the redirect.
        ...
        return req.session.save((saveErr) => {
          if (saveErr) return next(saveErr);
          const target =
            `https://aquiplex.com/auth/native/return?code=${encodeURIComponent(code)}` +
...
// ── Native Google OAuth return ────────────────────────────────────────────────
// The Android App Link intercepts this URL. If App Link verification fails,
// the browser can still open it and gets a harmless fallback page.
app.get("/auth/native/return", (req, res) => {
  res.status(200).type("html").send(`<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>Open Aqua</title>
<meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body><p>Authentication complete. Open the Aqua app to finish signing in.</p></body>
</html>`);
});

// The WebView makes this request, so the resulting aidex_session cookie lands
// in the WebView's own cookie jar. The code is single-use and expires quickly.
app.get("/auth/native/complete", async (req, res) => {
  const code = typeof req.query.code === "string" ? req.query.code : "";
  if (!code || code.length > 128) return res.redirect("/aqua?auth_error=invalid_code");

  try {
    const codeHash = crypto.createHash("sha256").update(code).digest("hex");
    const now = new Date();

    // Atomic burn: only one request can win the usedAt:null race.
    const record = await NativeOAuthCode.findOneAndUpdate(
      { codeHash, usedAt: null, expiresAt: { $gt: now } },
      { $set: { usedAt: now } },
      { new: true },
    );

    if (!record) return res.redirect("/aqua?auth_error=invalid_code");

    const user = await User.findById(record.userId);
    if (!user) return res.redirect("/aqua?auth_error=invalid_code");

    // Prevent session fixation: the browser session that performed OAuth is
    // discarded; the WebView receives a fresh authenticated session.
    await new Promise((resolve, reject) => {
      req.session.regenerate((err) => (err ? reject(err) : resolve()));
    });

    req.login(user, { session: true }, (loginErr) => {
      if (loginErr) return res.redirect("/aqua?auth_error=invalid_code");
      ...
      return req.session.save((saveErr) => {
        if (saveErr) return res.redirect("/aqua?auth_error=invalid_code");
        return res.redirect("/aqua");
      });
    });
  } catch (err) {
    console.error("[AUTH] Native OAuth completion failed:", err);
    return res.redirect("/aqua?auth_error=invalid_code");
  }
});

// App Links verification. The upload-key fingerprint covers locally installed
// builds. Play-distributed builds additionally require the Play App Signing
// SHA-256 fingerprint in PLAY_APP_SIGNING_SHA256.
app.get("/.well-known/assetlinks.json", (req, res) => {
  const fingerprints = [
    process.env.PLAY_APP_SIGNING_SHA256,
    process.env.AQUA_UPLOAD_SHA256 || "15:F7:A9:A8:72:79:D4:39:1F:DE:E5:5A:7E:02:B0:D2:9D:56:57:CB:AC:17:F0:CA:0F:ED:61:F4:B1:40:1C:2B",
  ].filter(Boolean);
