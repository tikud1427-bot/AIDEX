...
function WorkspaceContextChip({ workspace }: { workspace: NonNullable<UiMessage['workspace']> }) {
  ...
  return (
    <div className="mb-2 flex flex-wrap items-center gap-x-2 gap-y-1 rounded-lg border border-border/70 bg-surface-secondary/60 px-2.5 py-1.5 text-micro text-foreground-secondary">
      <span className="flex items-center gap-1 font-medium text-foreground-secondary">
        <FolderGit2 className="h-3 w-3" />
        Answering from your project · {name}
      </span>
      {files.length > 0 && (
        <span className="flex min-w-0 flex-wrap items-center gap-1">
          {files.slice(0, 4).map((f) => (
            <code key={f} className="truncate rounded bg-surface px-1.5 py-0.5 font-mono text-micro" title={f}>
              {f.split('/').pop()}
            </code>
          ))}
      ...
      >
        <div className="flex max-w-[85%] flex-col items-end gap-1 sm:max-w-[70%]">
          {editing ? (
            ...
            <div className="rounded-xl bg-primary px-4 py-2.5 text-lead text-primary-foreground shadow-sm [overflow-wrap:anywhere]">
              {message.content}
            </div>
          )}
          ...
          {!editing && message.attachments && message.attachments.length > 0 && (
            <div className="flex flex-wrap justify-end gap-1.5">
              {message.attachments.map((a) => (
                <span
                  key={a.id}
                  className="flex items-center gap-1.5 rounded-md border border-border bg-surface-secondary px-2 py-1 text-micro text-foreground-secondary"
                >
          ...
          {!editing && (
            <div className="hover-reveal flex items-center gap-1.5 pr-0.5">
              <span className="text-micro text-foreground-secondary/60">{formatTime(message.ts)}</span>
              {!generating && (
                <button
        ...
        {isThinking ? (
          ...
          <>
            ...
            {message.stoppedByUser && (
              <div className="mt-1.5 flex items-center gap-1.5 text-caption text-foreground-secondary/70">
            ...
            {!isStreaming && message.truncated && !generating && (
              <div className="mt-2 flex items-center gap-2">
                ...
                <span className="text-micro text-foreground-secondary/70">
            ...
            {!isStreaming && (
              <div className="mt-0.5 flex items-center gap-1">
                ...
                />
                <span className="hover-reveal ml-1 text-micro text-foreground-secondary/50">
